...
const PythonCodeView = ({ code }: { code: string }) => {
  ...
  return (
    <div className="font-mono text-[12px] leading-[20px] overflow-auto h-full">
      <table className="w-full border-collapse">
        <tbody>
          {lines.map((line, i) => (
            <tr key={i} className="hover:bg-white/[0.02]">
              <td className="text-right text-text-muted/50 select-none px-4 py-0 w-[1%] whitespace-nowrap align-top">
                {i + 1}
              </td>
...
const PythonLine = ({ line }: { line: string }) => {
  ...
  if (commentPart) {
    parts.push({ text: commentPart, className: "text-text-muted/60 italic" });
  }
...
const KEYWORDS = new Set([
  ...
  "for", "in", "not", "and", "or", "is", "None", "True", "False",
  "with", "as", "try", "except", "finally", "raise", "yield", "lambda",
  "global", "nonlocal", "pass", "break", "continue", "while",
]);

const BUILTINS = new Set([
  "print", "len", "range", "int", "float", "str", "list", "dict",
  "set", "tuple", "round", "sum", "abs", "max", "min", "sorted",
...
const tokenize = (code: string): { text: string; className: string }[] => {
  ...
  const regex = /("""[\s\S]*?"""|'''[\s\S]*?'''|f"[^"]*"|"[^"]*"|'[^']*'|@\w[\w.]*|\b\d+\.?\d*\b|\b\w+\b|[^\w\s]+|\s+)/g;
  ...
  while ((match = regex.exec(code)) !== null) {
    ...
    if (token.startsWith('"""') || token.startsWith("'''")) {
      result.push({ text: token, className: "text-emerald-400/70" });
    } else if (token.startsWith('"') || token.startsWith("'") || token.startsWith('f"')) {
      result.push({ text: token, className: "text-emerald-400/70" });
    } else if (token.startsWith("@")) {
      result.push({ text: token, className: "text-amber-400/80" });
    } else if (/^\d/.test(token)) {
      result.push({ text: token, className: "text-orange-300/80" });
    } else if (KEYWORDS.has(token)) {
      result.push({ text: token, className: "text-violet-400/80" });
    } else if (BUILTINS.has(token)) {
      result.push({ text: token, className: "text-sky-400/70" });
    } else if (/^\w+$/.test(token)) {
      result.push({ text: token, className: "text-text-primary/80" });
...
const DetailPanel = ({
  ...
  const [activeTab, setActiveTab] = useState<"code" | "runs" | "config">("code");
  const status = statusConfig[pipeline.status];
    ...
    <div className="flex flex-col h-[calc(100vh-120px)] bg-bg-secondary border border-border-subtle rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-border-subtle shrink-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button className="flex items-center gap-1.5 text-[12px] text-accent hover:text-accent-hover font-medium transition-colors px-2.5 py-1 rounded-md hover:bg-accent/5">
            <Play size={12} />
            Run Now
          </button>
          <button
      ...
      <div className="flex items-center gap-1 px-5 pt-2 border-b border-border-subtle shrink-0">
        {(["code", "runs", "config"] as const).map((tab) => (
          ...
          >
            {tab === "code" ? "Code" : tab === "runs" ? "Run History" : "Configuration"}
          </button>
        ))}
      </div>
        ...
        {activeTab === "code" && (
          <div className="h-full flex flex-col">
            ...
            <div className="flex items-center justify-between px-5 py-2 border-b border-border-subtle bg-bg-primary/50 shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button className="text-text-muted hover:text-text-secondary transition-colors p-1" title="Copy code">
                  <Copy size={13} />
                </button>
              </div>
            </div>
          ...
          <div className="p-5 overflow-auto h-full space-y-2">
            {pipeline.recentRuns.map((run) => (
              ...
              >
                ...
                <button className="text-text-muted hover:text-text-secondary transition-colors p-0.5" title="Replay">
                  <RotateCcw size={12} />
                </button>
              </div>
            ))}
        ...
        {activeTab === "config" && (
          <div className="p-5 overflow-auto h-full space-y-5">
            ...
            <div>
              ...
              <span className="text-[13px] text-text-primary font-mono">
                {pipeline.nextRun === "-" ? "\u2014" : timeAgo(pipeline.nextRun)}
              </span>
            </div>
          </div>
        )}
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="flex items-center gap-1 text-[10px] font-mono text-text-secondary">
                <span className={`w-1.5 h-1.5 rounded-full ${clusterStatus.dotColor}`} />
                {clusterStatus.label}
              </span>
            </div>
...
const PipelinesPage = () => {
  ...
  const [activeCluster, setActiveCluster] = useState<ComputeCluster>(computeClusters[0]);
